...
void addFeedback(MastermindWindow& mwin, const int correctPosition, const int correctCharacter, const int size, const int round)
{
	...
	const int centerY = static_cast<int>(((round*3*guessHeight/2))*mwin.y_max()); // Regn ut sirklenes y-koordinat (sentrum)
	
	for (int i = 0; i < size; ++i) {
		const int centerX = static_cast<int>( (guess_w + i*feedback_w/size)*mwin.x_max() ); // Regn ut sirkelens x-koordinat (sentrum)

		mwin.vc.push_back(new Circle{ Point{centerX, centerY}, circleRadius});
